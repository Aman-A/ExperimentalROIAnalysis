function [vol]=imreadBF(datname,zplanes,tframes,channels)
...
% Modifed by Aman Aberra to make compatible with multi-color, multi-time point z-stack
% aman.s.aberra@dartmouth.edu
% Aug 2022
...
if nargin < 3 || isempty(tframes)
    tframes = 1:r.getSizeT();
end
if nargin < 2 || isempty(zplanes)
    zplanes = 1:r.getSizeZ(); 
end
...
vol=zeros(height,width,length(zplanes),length(channels),length(tframes));

for i = 1:length(zplanes)
    for j = 1:length(channels)
        for k = 1:length(tframes)
            index = r.getIndex(zplanes(i)-1,channels(j)-1,tframes(k)-1); % zero-index
