...
function baseline = asLS_baseline(signal, smoothness_param, asym_param)
...
if nargin == 1
    smoothness_param = 10; % Specify the smoothness of the baseline. 
                            % The smoothing factor must be greater than 0, 
                            % and the typical value is 2 and 9. The larger the smoothing factor is, 
                            % the smoother the baseline will be.
    % from https://www.originlab.com/doc/Origin-Help/PeakAnalyzer-ALSBaseline
    asym_param = 0.1;  % Specify weight of points above the baseline in each iteration. 
                        % For positive peaks it should be close to 0, and close to 1 for negative peaks. 
                        % The asymmetric factor must be between 0 and 1. 
                        % The smaller the asymmetric factor is, the less effect points 
                        % above the baseline have on baseline in next iteration. 
                        % The asymmetric factor should be chosen close to 0 for positive peaks, eg., 0.001, 
                        % and close to 1 for negative peaks, eg.,0.99
