...
[~,~,diff_img1,~] = diffImage(rec1,exp_settings,'include_plots',...
                            in.include_plots);                         
diff_img_stack = zeros(size(diff_img1,1),size(diff_img1,2),num_trials);
diff_img_stack(:,:,1) = diff_img1; 
% Load rest of images 
trial_ind = 2;
...
for i = 1:length(trial_folders)    
    ...
    for j = startj:length(img_namesi)
        ...
        rec_ij.load(); 
        [~,~,diff_img,~] = diffImage(rec_ij,exp_settings,'include_plots',...
                                    in.include_plots); 
        diff_img_stack(:,:,trial_ind) = diff_img; 
        trial_ind = trial_ind + 1; 
    end
end
% Save image stack
path_to_diff_img_stack = fullfile(exp_folder,[in.img_stack_name '.fits']);
fitswrite(diff_img_stack,path_to_diff_img_stack); 
