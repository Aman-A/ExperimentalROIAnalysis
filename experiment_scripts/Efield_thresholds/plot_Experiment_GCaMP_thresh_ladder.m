...
ps.spike_thresh = 8; % 6 x std
ps.spike_window = 0.1; % sec

pulse_dur_ms = 0.05; % pulse duration in ms  
E_per_mA = 68.524; % V/m per mA 
% E_per_mA = 65.27;
isolator_mA_per_V = 10; 

trial_ind = 1; 
E_recs_fold = sprintf('E_recs_%gms',pulse_dur_ms);
...
if isempty(above_thresh_amps)
    fprintf('Threshold for %g ms pulse above %.3f V/m (%.3f mA)\n',...
            exp_settings.stim_pulse_dur_sec*1e3,amps(end)*exp_settings.E_per_mA,amps(end))
elseif above_thresh_amps_inds(1) == 1
    fprintf('Threshold for %g ms pulse below %.3f V/m (%.3f mA)\n',...
            exp_settings.stim_pulse_dur_sec*1e3,amps(1)*exp_settings.E_per_mA,amps(1))
else
    ...
    fprintf('Threshold for %g ms pulse between %.3f to %.3f V/m (%.2f %% window, %.3f mA)\n',...
        exp_settings.stim_pulse_dur_sec*1e3,lower_lim_threshE,threshE,thresh_window_per_size,...
        amps(above_thresh_amps_inds(1)))
end
%% Plot multiple trials, same condition
pulse_durs_ms_all = [0.05,0.1,0.2,0.5,1,5];
% pulse_durs_ms_all = 0.05; 
trial_inds_all = {1:5,1:5,1:5,1:5,1:5,1:8}; 
threshEs = nan(length(pulse_durs_ms_all),1);
for i = 1:length(pulse_durs_ms_all)
    pulse_dur_ms = pulse_durs_ms_all(i); % pulse duration in ms     
    ps.condition = sprintf('thresh_%gms',pulse_dur_ms); % 
    E_recs_fold = sprintf('E_recs_%gms',pulse_dur_ms);
    trial_inds = trial_inds_all{i}; 
    stim_files = numericVec2chars(trial_inds,sprintf('%s/E_trial_%%04d.h5',E_recs_fold));    
    img_names = arrayfun(@(x) reindexFileNameForAndor('trial',x),trial_inds,'UniformOutput',0);
    ...
    % Plot threshold figs
    thresh_out = analyzeThresholdLadderTrials(exp_settings_all(1).stim_amps,successful_spikes_all,...
