...
stim_vals2 = 10.25; % sec - time of dc pulse
...
%%
ps.condition = 'control_4mABi_-1mAG';
img_name = [ps.condition '.fits'];
trace_fig = figure('Units','normalized'); 
trace_fig.Position(1:2) = [1 0.4]; 
trace_axis = gca;
datai = plotTrial(img_name,exp_settings,roiset_filename,...
                   trace_axis,ps);
%% Plot trials within each condition
ps.condition = 'control_3mABi_-1mAG'; % 'control', '5nM_DTX', '50nM_DTX'
...
amps = [-1 1];
conditions = arrayfun(@(x) sprintf('control_4mABi_%gmAG',x),amps,'UniformOutput',0);
