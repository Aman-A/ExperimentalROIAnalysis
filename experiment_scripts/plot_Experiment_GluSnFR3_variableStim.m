...
for i = 1:num_freqs    
    durs(i) = num_stim/freqs(i);     
    stim_vals = defineStimTrain(del,freqs(i),durs(i));
...
img_name = 'stim_2Hz_sampling_200Hz'; 
freq_plot = 2; 
trace_fig = figure; 
trace_axis = gca;
datai = plotTrial(img_name,exp_settings(freqs == freq_plot),roiset_filename,...
                   trace_axis,ps);
%% Plot multiple trials, same condition
ps.condition = 'control'; % 'control', '50nM_DTX'
img_names = {'stim_2Hz_sampling_200Hz'}; % use all images in condition folder
