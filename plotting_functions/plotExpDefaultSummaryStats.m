...
if any(in.plot_inds == 4)
    ...
    plotSummaryStats(out.baselines_all,[],out.conditions,...
                    ...
                    'fig_dir',summary_fig_dir,'title_on',in.title_on);            
    subplot(2,1,2)
    baselines_all_rel = cellfun(@(x) 100*((mean(x,2:4)-mean(out.baselines_all{1},2:4))./mean(out.baselines_all{1},2:4)),...
...
if any(in.plot_inds == 7)
    ...
    %                                      peak_times_all,...
    %                                      'UniformOutput',0);
%     mean_fwhm = cellfun(@(x) x*1e3,out.mean_fwhm,'UniformOutput',0); 
    norm_fwhm = cellfun(@(x) x./mean(out.mean_fwhm{1}),out.mean_fwhm,'UniformOutput',0); 
