...
% AUTHOR    : Aman Aberra 
in.AP_window = 20e-3; % time window (or frames) around AP peak to extract, specify units below
                      % E.g. 50e-3 gives 25 ms before and after peak of
                      % each AP
in.AP_window_units = 'sec'; % specify units of AP_window, 'sec' or 'frames'
in.min_interAP_interval = 0.9; % min time between APs defined as proportion of inter-stimulus interval
in.min_AP_width = 0.2e-3; % min AP width (sec)
in.plot_roi_ind = 1; % specify ROIs to plot in 2nd subplot (averaged waveforms overlaid)
in.font_size = 16; 
in.invert = 0; % invert if finding peak hyperpolarization
in.print_level = 1;
...
frames_post_stim = frames(frames>stim_frames(1)-10); % allow for defined 
                                                      % stim frame to be inaccurate, 
                                                      % some responses may occur a little before
% means = detrend(means);      
means_post_stim = means(frames>stim_frames(1)-10,:);
...
for i = 1:num_rois
    if in.invert
       mean_post_stimi = -means_post_stim(:,1); 
    end
    [peak_vals,peak_frames] = findpeaks(mean_post_stimi,frames_post_stim,...
...
for j = 1:num_rois
    ...
    deltaF_F0_all{j} = (AP_framesj - baselines(1:length(peak_vals_all{j}),j)')./baselines(1:length(peak_vals_all{j}),j)'; % get DF/F using AP-specific baseline (for jth roi)
end
...
% Plot
cols = {'k','r','g','b'};
light_cols = {0.4*ones(1,3),0.4*[1 0 0],0.4*[0 1 0],0.4*[0 0 1]};
figure; 
for i = 1:num_rois
    coli = cols{i}; 
    light_coli = light_cols{i};
    ax1 = subplot(2,1,1);
    plot(ax1,frames,means(:,i),'Color',coli); hold on;
    plot(ax1,peak_frames_all{i},peak_vals_all{i},'o','Color',light_coli);
...
%     plot(ax1,1e3*peak_frames_all{i}/exp_settings.sampling_rate,peak_vals_all{i},'o','Color',light_coli);
    ...
    ax1.XLim = [stim_frames(1)-100,frames(end)];
    xlabel(ax1,'Frames'); 
...
plot(ax1,stim_frames,ax1.YLim(2)*0.99*ones(1,length(stim_frames)),...
    'r.','MarkerSize',8,'DisplayName','Stim times'); 
if num_rois == 1
    % convert cell array to matrices
    mean_APs_all = mean_APs_all{1}; 
    deltaF_F0_all = deltaF_F0_all{1};
    peak_frames_all = peak_frames_all{1}; 
end
end