function [isSignificant,adjusted_pvals,alpha]= bonferroni_holm(pvals,optional_alpha)
    ...
    pval_vector = pvals(:); 
    nan_inds = isnan(pvals);
    pval_vector = pval_vector(~nan_inds); % remove nans
    C = length(pval_vector);
    ...
    adjusted_pval_vector_full = nan(numel(pvals),1);
    adjusted_pval_vector_full(~nan_inds) = adjusted_pval_vector; 
    isSignificant_vector_full = nan(numel(pvals),1);
    isSignificant_vector_full(~nan_inds) = isSignificant_vector; 
    % adjusted_pvals = reshape(adjusted_pval_vector, size(pvals));
    % isSignificant  = reshape(isSignificant_vector, size(pvals));
    adjusted_pvals = reshape(adjusted_pval_vector_full, size(pvals));
    isSignificant  = reshape(isSignificant_vector_full, size(pvals));
end