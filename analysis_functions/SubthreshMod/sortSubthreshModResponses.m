...
in.sort_by_mode = 1; % 1 - use max abs modulation to sort, 2 - use 1st and last condition (i.e., longest duration or highest intensity)
...
for i = 1:num_rois_total
    ...
    if ( (out.peak_mod_during_per(i,max_ind) > 0 && plot_amps(max_ind) < 0) ... % negative current facilitates
        ...
        out.peaks_before_mat(i,:,:) = out.peaks_before_mat(i,:,num_durs:-1:1);
        out.peaks_during_mat(i,:,:) = out.peaks_during_mat(i,:,num_durs:-1:1);
        for j = 1:num_durs
...
for i = 1:num_dishes
    % [~,max_ind] = max(abs(peak_mod_during_cell_per(i,:)));
    [~,max_ind] = max(abs(peak_mod_during_cell_per(i,dc_inds)));
    max_ind = dc_inds(max_ind);
    if ( (peak_mod_during_cell_per(i,max_ind) > 0 && plot_amps(max_ind) < 0) ... % negative current facilitates
            ||  (peak_mod_during_cell_per(i,max_ind) < 0 && plot_amps(max_ind) > 0)) % or positive current suppresses
