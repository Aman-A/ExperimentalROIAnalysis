    ...
    for jp = 1:length(plot_amps)  %length(amp_indsi)        
        if any(plot_amps(jp) == subthresh_amps{i})
            ...
            if num_trials >= min_num_trials_per_amp                            
                ...
                peaks_before{peak_indj} = [[peaks_before{peak_indj},nan(size(peaks_before{peak_indj},1),max_num_peaks-size(peaks_before{peak_indj},2))];...
                                             [squeeze(peaksij(1,:,:)),...
                                             nan(size(peaksij,2),max_num_peaks-num_peaksij)]];
                peaks_during{peak_indj} = [[peaks_during{peak_indj},nan(size(peaks_during{peak_indj},1),max_num_peaks-size(peaks_during{peak_indj},2))];...
                                             [squeeze(peaksij(2,:,:)),...
                                             nan(size(peaksij,2),max_num_peaks-num_peaksij)]];
                peaks_after{peak_indj} = [[peaks_after{peak_indj},nan(size(peaks_after{peak_indj},1),max_num_peaks-size(peaks_after{peak_indj},2))];...
                                             [squeeze(peaksij(3,:,:)),...
                                             nan(size(peaksij,2),max_num_peaks-num_peaksij)]];            
                ...
                if ~isempty(dF_al2_before{peak_indj}) && max_num_peaks > size(dF_al2_before{peak_indj},3) % pad experiments with fewer trials with nans
                    dF_al2_before{peak_indj} = cat(3,dF_al2_before{peak_indj},...
                        nan([size(dF_al2_before{peak_indj},[1 2]),max_num_peaks-size(dF_al2_before{peak_indj},3)]));
                    dF_al2_during{peak_indj} = cat(3,dF_al2_during{peak_indj},...
                        nan([size(dF_al2_during{peak_indj},[1 2]),max_num_peaks-size(dF_al2_during{peak_indj},3)]));
                    dF_al2_after{peak_indj} = cat(3,dF_al2_after{peak_indj},...
                        nan([size(dF_al2_after{peak_indj},[1 2]),max_num_peaks-size(dF_al2_after{peak_indj},3)]));
                end
                dF_al2_beforeij = reshape(dF_al2ij(:,:,1,:),...
                                    [size(dF_al2ij,[1,2]),num_peaksij]);
                dF_al2_before{peak_indj} = [dF_al2_before{peak_indj},...
                                            cat(3,dF_al2_beforeij,nan([size(dF_al2_beforeij,[1,2]),max_num_peaks-size(dF_al2_beforeij,3)]))];
                dF_al2_duringij = reshape(dF_al2ij(:,:,2,:),...
                                    [size(dF_al2ij,[1,2]),num_peaksij]);            
                dF_al2_during{peak_indj} = [dF_al2_during{peak_indj},...
                                            cat(3,dF_al2_duringij,nan([size(dF_al2_duringij,[1,2]),max_num_peaks-size(dF_al2_duringij,3)]))];
                dF_al2_afterij = reshape(dF_al2ij(:,:,3,:),...
                                    [size(dF_al2ij,[1,2]),num_peaksij]);            
                dF_al2_after{peak_indj} = [dF_al2_after{peak_indj},...
                                            cat(3,dF_al2_afterij,nan([size(dF_al2_afterij,[1,2]),max_num_peaks-size(dF_al2_afterij,3)]))];                
                dish_inds{peak_indj} = [dish_inds{peak_indj};i*ones(num_roisi,1)];
                
            else                
                peaks_before{peak_indj} = [peaks_before{jp};nan(num_roisi,size(peaks_before{jp},2))];
                peaks_during{peak_indj} = [peaks_during{jp};nan(num_roisi,size(peaks_during{jp},2))];
                peaks_after{peak_indj} = [peaks_after{jp};nan(num_roisi,size(peaks_after{jp},2))];
                dF_al2_before{peak_indj} = [dF_al2_before{peak_indj},...
                                            nan(size(dF_al2_before{peak_indj},1),num_roisi,max_num_peaks)];
                dF_al2_during{peak_indj} = [dF_al2_during{peak_indj},...
                                            nan(size(dF_al2_before{peak_indj},1),num_roisi,max_num_peaks)];
                dF_al2_after{peak_indj} = [dF_al2_after{peak_indj},...
                                            nan(size(dF_al2_before{peak_indj},1),num_roisi,max_num_peaks)];
            ...
            if isempty(included_ampsi)
                ...
                peaks_before{jp} = [peaks_before{jp};nan(num_roisi,size(peaks_before{jp},2))];
                peaks_during{jp} = [peaks_during{jp};nan(num_roisi,size(peaks_during{jp},2))];
                peaks_after{jp} = [peaks_after{jp};nan(num_roisi,size(peaks_after{jp},2))];
...
fprintf('Extraced peaks and stim-aligned traces in %g dishes, %g ROIs total\n',num_dishes,sum(num_rois))
end