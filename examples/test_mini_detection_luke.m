...
exp_folder = fullfile(getDataFold('luke'),'220802.2');
trial_folder = fullfile(exp_folder,'GluSnFR sucrose');
rec_name = ''; % name of recording file, if left empty, grabs first file in trial_folder
...
if isempty(preset_name)
    ...
    settings.min_mini_width = 0; % sec - minimum peak width, 
                                                % using findpeaks.m definition of width (default 'halfprom')
    settings.min_peak_distance = 0.05; % sec - min distance between mini peaks to allow
    % Filtering
    ...
    settings.num_frames_skip_start_end = round(0.2*sampling_rate); % frames to remove from start/end due to filtering artifacts
