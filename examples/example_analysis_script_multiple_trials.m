...
ps.filt_width = 0;
ps.funcs = {'baseline','deltaF_F0'}; % options: 'mean','std','baseline','deltaF_F0'
ps.roi_func_mode = 'separate'; % options: 'combine' or 'separate'
ps.recenterROIs = 0; % set to 0 for off, or set to 'diff', 'peak', or 'baseline' to use those images for recentering ROIs
ps.save_processed_data = 1;
ps.load_processed_data = 0;
ps.save_fig = 0;    
ps.roiset_filedir = img_folder; % file directory where roisets are saved, currently same as image files
%% Specify your experimental parameters
sampling_rate = 200; % Sampling rate (frames/sec or Hz)
stim_vals = [50]; % Stimulation frames, either a single value or list of
