...
dish = 'dish06'; 
cond_name = 'control';
rec_name = 'GluSnFR(00001).tif'; % name of recording file, if left empty, grabs first file in trial_folder

exp_folder = fullfile(getDataFold('emma'),'thesis_experiments',exp_date,...
                        'GluSnFR3_SynmRuby',dish);
trial_folder = fullfile(exp_folder,cond_name);


% 2) Specify ROI set zip file (output of saving ROIs in ImageJ)

roiset_name = 'RoiSet.zip';
...
units = 'frames'; % 'sec' or 'frames'
exp_settings = ExperimentSettings(stim_times,stim_wind,baseline_wind,units,...
                                    sampling_rate); 
...
if isempty(preset_name)
    ...
    settings.nframes_forward = round(0.48*sampling_rate); % number of frames after each mini peak to extract
    settings.mini_width_range = [1/sampling_rate 0.3]; % sec - range of width 
    settings.mini_width_frac_amp = 0.5; % fraction of amplitude to calculate width, e.g. 0.5 is full width half max (FWHM)
    settings.min_peak_distance = 1/sampling_rate; % sec - min distance between mini peaks to allow
    settings.mini_amp_range = [0.06 inf]; % deltaF/F - amplitudes to exclude
    ...
    settings = detectMinisPresets(preset_name,sampling_rate);
end
 % plotting and file saving
settings.plot_filt_output_roi_index = 6; % plot example of filtering in this roi
